...
.. csv-table:: Operator Executor Compatibility Matrix
   ...
   "fft2", "|yes|", "|yes|", "|yes|", "Host support requires the CPU FFT backend. CUDAJITExecutor support uses cuFFTDx through MathDx for supported 2D runtime shapes, precisions, and layouts."
   ...
   "ifft2", "|yes|", "|yes|", "|yes|", "Host support requires the CPU FFT backend. CUDAJITExecutor support uses cuFFTDx through MathDx for supported 2D runtime shapes, precisions, and layouts."
   ...
   "random", "|yes|", "|yes|", "|no|", "Direct CUDA tensor assignment uses a low-memory fill path reproducible for the same shape/layout/path; generic random expressions still use generator state and are not JIT-fused."
   "randomi", "|yes|", "|yes|", "|no|", "Direct CUDA tensor assignment uses a low-memory fill path reproducible for the same shape/layout/path; generic random integer expressions still use generator state and are not JIT-fused."
