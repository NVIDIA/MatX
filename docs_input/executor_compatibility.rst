...
.. csv-table:: Operator Executor Compatibility Matrix
   ...
   "random", "|yes|", "|yes|", "|no|", "Direct CUDA tensor assignment uses a low-memory fill path; generic random expressions still use generator state and are not JIT-fused."
   "randomi", "|yes|", "|yes|", "|no|", "Direct CUDA tensor assignment uses a low-memory fill path; generic random integer expressions still use generator state and are not JIT-fused."
